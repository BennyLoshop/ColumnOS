...
(async () => {
  // 提取 URL 中的 JWT（apiToken 参数）
  const params = new URLSearchParams(location.search);
  const apiToken = params.get("apiToken");

  if (!apiToken) {
    await alert("缺少 apiToken 参数！");
    return;
  }

  // 从 JWT 中提取唯一标识（name claim）
  ...
  if (!user) {
    alert("JWT 无效或未包含唯一标识字段！");
    return;
  }

  // 尝试从 localStorage 读取激活码
  let code = localStorage.getItem("activation_code");

  const socket = new WebSocket("ws://115.29.169.60:6789");

  socket.onopen = () => {
    socket.send(apiToken);
  };
  ...
  async function askCode() {
    const input = await customPrompt("请输入16位激活码,获取：http://u5a.cn/KNhvW");
